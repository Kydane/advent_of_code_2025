...
public class Day8Solution {
    ...
    private static void connectJunctions( int totalConnections) {
        ...
        while (newConnections < totalConnections){
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<Circuit> circuits = circuitMap.values().stream().collect(Collectors.toCollection(HashSet::new)).stream().sorted((o1, o2) -> Integer.compare(o2.getJunctionBoxes().size(), o1.getJunctionBoxes().size())).collect(Collectors.toCollection(ArrayList::new));

        int[] circuitSize = new int[3];
        for (int i = 0; i < 3; i++) {
            circuitSize[i] = circuits.get(i).getJunctionBoxes().size();
        }
        int value = circuitSize[0]*circuitSize[1]*circuitSize[2];
        System.out.printf("%nOutput: %s%n ", value);
